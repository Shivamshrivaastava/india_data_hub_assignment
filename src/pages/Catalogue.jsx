...
function Catalogue() {
  ...
  */
  ...
  /*
    Normalize India dataset
  */
  const normalizedIndia = useMemo(() => {
    return (categoriesData.frequent || []).map((item, index) => ({
      id: item.id || index,
      name: item.title,
      category: item.cat || item.subCat || "—",
      range: item.freq || "—",
  ...
  /*
    Select dataset
  */
  const baseData = selectedDataset === "IMF" ? normalizedIMF : normalizedIndia;
